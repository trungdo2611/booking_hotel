...
public class RoomController {
    ...
    public ResponseEntity<Response> addNewRoom(
        ...
        if (photo == null || photo.isEmpty() || roomType == null || roomType.isBlank() || roomCode == null || roomCode.isBlank() || roomPrice == null || roomDescription.isBlank()) {
            Response response = new Response();
            response.setStatusCode(400);
            response.setMessage("Please provide values for all fields(photo, roomType,roomCode,roomPrice, description)");
            return ResponseEntity.status(response.getStatusCode()).body(response);
        }
        Response response = roomService.addNewRoom(photo, roomType,roomCode , roomPrice, roomDescription);
        return ResponseEntity.status(response.getStatusCode()).body(response);
    }
    ...
    public ResponseEntity<Response> updateRoom(@PathVariable Long roomId,
        ...
        Response response = roomService.updateRoom(roomId, roomDescription, roomType,roomCode, roomPrice, photo);
        return ResponseEntity.status(response.getStatusCode()).body(response);
    }
