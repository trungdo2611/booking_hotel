...
export default class ApiService {
  ...
  static async bookRoom(roomId, userId, booking, totalAmount) {
    console.log("USER ID IS: " + userId);

    const response = await axios.post(
      `${this.BASE_URL}/bookings/book-room/${roomId}/${userId}?totalAmount=${totalAmount}`,
      booking,
      {
        headers: this.getHeader(),
      }
  ...
  // Get api chatbot AI
  static async getChatbot(chatId, formData) {
    const result = await axios.post(
      `${this.BASE_URL}/chat/stream?chatId=${chatId}`,
      formData
    );
    return result.data;
  }
  ...
  //post api paypal
  static async creatPayment(total, currency, method, intent, description,cancelUrl, url) {
    const result = await axios.post(
      `${this.BASE_URL}/paypal/create-payment?total=${total}&currency=${currency}&method=${method}&intent=${intent}&description=${description}&cancelUrl=${cancelUrl}&url=${url}`
    );
    return result.data;
  }
  ...
  static async executePayment(paymentId, payerId) {
    const result = await axios.get(`${this.BASE_URL}/paypal/execute-payment?paymentId=${paymentId}&payerId=${payerId}`);
    return result.data;
  }
